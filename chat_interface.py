...
class ChatInterface:
    ...
    def send_message(self):
        ...
        # Use a thread to process the message
        threading.Thread(
    ...
    async def send_message(self, message):
        try:
            ...
            self.add_message(Message(content=message, sender="user"))
            self.add_message(Message(content=response, sender="system"))
            
            return response
        except Exception as e:
            self.hide_typing_indicator()
            error_msg = f"Error: {str(e)}"
            self.add_message(Message(content=error_msg, sender="system", message_type="error"))
            return error_msg
    ...
    def process_message(self, message: str):
        ...
        # Process the message
        result = self.conversation_manager.process_user_input(message)
        
        # Update UI
        self.root.after(0, self.handle_response, result)
